<#
.SYNOPSIS
    tests-async Installer for Windows PowerShell
    Verifies prerequisites for test orchestration with TUnit
#>

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "tests-async Installer (Windows PowerShell)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$delegatedSkills = @("tdd-async", "bdd-async", "test-fixer-async")
...
foreach ($skill in $delegatedSkills) {
    ...
    if (Test-Path $skillPath -PathType Container) {
        ...
        Write-Host "⚠️  Missing delegated skill: $skill" -ForegroundColor Yellow
    }
}
...
Write-Host "2. Follow the orchestration workflow in SKILL.md" -ForegroundColor Gray
